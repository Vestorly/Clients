...
class AdvisorsApi(object):
    ...
    def find_advisor_by_id(self, vestorly_auth, id, **kwargs):
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.find_advisor_by_id(vestorly_auth, id, callback=callback_function)
        ...
        if kwargs.get('callback'):
            return self.find_advisor_by_id_with_http_info(vestorly_auth, id, **kwargs)
        else:
            (data) = self.find_advisor_by_id_with_http_info(vestorly_auth, id, **kwargs)
            return data

    def find_advisor_by_id_with_http_info(self, vestorly_auth, id, **kwargs):
        """
        Returns a single advisor given their ID
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please define a `callback` function
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.find_advisor_by_id_with_http_info(vestorly_auth, id, callback=callback_function)
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    ...
                    " to method find_advisor_by_id" % key
                )
            params[key] = val
        del params['kwargs']

...
class MemberEventsApi(object):
    ...
    def find_member_events(self, vestorly_auth, **kwargs):
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.find_member_events(vestorly_auth, callback=callback_function)
        ...
        if kwargs.get('callback'):
            return self.find_member_events_with_http_info(vestorly_auth, **kwargs)
        else:
            (data) = self.find_member_events_with_http_info(vestorly_auth, **kwargs)
            return data

    def find_member_events_with_http_info(self, vestorly_auth, **kwargs):
        """
        Returns all MemberEvents
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please define a `callback` function
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.find_member_events_with_http_info(vestorly_auth, callback=callback_function)
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    ...
                    " to method find_member_events" % key
                )
            params[key] = val
        del params['kwargs']
